...
import json
import time
from hr_automation import generate_and_send_completion_email
...
def get_db_connection():
    try:
        ...
        st.error(
            f"Database Connection Error: {e}. Please ensure the database is running."
        )
        st.stop()
...
def update_document_status(candidate_id, doc_key, status, file_path=None):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "SELECT document_status FROM candidates WHERE id = %s;", (candidate_id,)
            )
            doc_status_json = cur.fetchone()["document_status"]
            doc_status_json[doc_key] = {
                "status": status,
                "path": file_path,
...
def show_offer_page():
    ...
    st.markdown(
        f"We are excited to offer you the position of **{candidate['role']}** starting on **{candidate['start_date']}**."
    )
    st.markdown("---")
    st.subheader("Your Offer Letter")
    st.markdown(candidate["offer_letter_content"], unsafe_allow_html=True)
    st.info(
        "Please review your offer carefully. To proceed, click one of the options below."
    )
...
def show_documents_page():
    st.success(
        f"Congratulations, {st.session_state.candidate['name']}! You have accepted your offer."
    )
    st.markdown(
        "Please upload the required documents below to complete your onboarding. The previous employment letter is optional."
    )
    ...
    with st.form("doc_upload_form"):
        ...
        for doc_label, doc_key in documents_required.items():
            ...
            current_status = (
                st.session_state.candidate["document_status"]
                .get(doc_key, {})
                .get("status", "Pending")
            )
        ...
        if submitted:
            ...
            with st.spinner("Processing your documents... Please wait."):
                ...
                if all_uploads_successful:
                    ...
                    for doc_key, file in uploaded_files.items():
                        if file:
                            doc_label = [
                                k for k, v in documents_required.items() if v == doc_key
                            ][0]
                            file_path = save_uploaded_file(
                                file, st.session_state.candidate["id"], doc_key
                            )
                            if file_path:
                                if update_document_status(
                                    st.session_state.candidate["id"],
                                    doc_key,
                                    "Submitted",
                                    file_path,
                                ):
                                    st.toast(
                                        f"✅ {doc_label} saved successfully!", icon="✅"
                                    )
                                    # Update session state immediately
                                    st.session_state.candidate[
                                        "document_status"
                                    ].setdefault(doc_key, {})["status"] = "Submitted"
                                else:
                                    st.toast(
                                        f"❌ DB update failed for {doc_label}.",
                                        icon="❌",
                                    )
                                    all_uploads_successful = False
                            else:
                                st.toast(
                                    f"❌ File save failed for {doc_label}.", icon="❌"
                                )
                                all_uploads_successful = False
            ...
            if all_uploads_successful and any(uploaded_files.values()):
                ...
                with st.spinner("Generating final welcome package..."):
                    completion_content = generate_and_send_completion_email(
                        st.session_state.candidate["id"]
                    )
                    if completion_content:
                        ...
                        st.error(
                            "Could not generate or send the final welcome email. Please contact HR."
                        )
                time.sleep(2)
                st.rerun()
                ...
                st.error(
                    "Some documents failed to process. Please correct them and try again."
                )
...
def show_completion_page():
    ...
    st.success(
        "You have successfully completed all the onboarding steps. We are thrilled to have you join us!"
    )
    st.markdown("---")
    ...
    if "completion_content" in st.session_state:
        ...
        st.info(
            "The message above has also been sent to your email for your reference."
        )
    else:
        st.warning(
            "Could not load completion message. Please check your email or contact HR."
        )


def show_rejected_page():
    st.error(
        f"Thank you for your response, {st.session_state.candidate['name']}. Your offer has been marked as Rejected."
    )
...
def main():
    ...
    if (
        datetime.now(st.session_state.candidate["token_expiry"].tzinfo)
        > st.session_state.candidate["token_expiry"]
    ):
